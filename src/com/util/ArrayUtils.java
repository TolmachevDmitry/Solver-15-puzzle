...
public class ArrayUtils {
    ...
    /**
     * Конвертация массива int в массив чисел типа Integer
     */
    public static Integer[] intToInteger(int[] arrOfInt) {
        Integer [] arrOfInteger = new Integer[arrOfInt.length];
        for (int i = 0; i < arrOfInt.length; i++) {
            arrOfInteger[i] = Integer.valueOf(arrOfInt[i]);
        }
     ...
     */
    public static int[] integerToInt(Integer[] arrOfInteger) {
        int[] arrOfInt = new int[arrOfInteger.length];
        for (int i = 0; i < arrOfInteger.length; i++) {
            arrOfInt[i] = Integer.parseInt("" + arrOfInteger[i]);
        }
    ...
    */
     ...
     * @param len Кол-во элементов в массиве
     * @return Массив int[]
     */
    public static int[] createRandomIntArray(int len) {
        ...
        for (int i = 0; i < len; i++) {
            double num = Math.random()*200 - 100;
            arr[i] = (int)num;
        }
        return arr;
    }

    public static int[][] createRandomUniqueIntArray(int n, int m) {
        HashSet<Integer> values = new HashSet<>();
        int[][] arr = new int[n][m];

        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                int value = toInteger(Math.random() * (n * m - 1) + 1);
                while (values.contains(value) || value == 0) {
                    value = toInteger(Math.random() * (n * m + 2) - 2);
                }
                values.add(value);
    ...
    public static int[][] copy(int[][] matrix) {
        int[][] otherArray = new int[matrix.length][];

        for (int i = 0; i < matrix.length; i++) {
            otherArray[i] = matrix[i].clone();
        }
